...
def test_出清_視為調節():
    ...
    # 完全出清（今天持股 0）→ 特別註記，跟一般調節分開看
    assert out["flags"]["1101"]["exit_notes"] == {"00991A": "出清"}


def test_賣出後僅剩1張_特別註記():
    out = b.build_flags({"00991A": _snap(
        prev=[("2330", "台積電", 1000_000, 50.0), ("2891", "中信金", 50_000, 5.0)],
        today=[("2330", "台積電", 1000_000, 55.0), ("2891", "中信金", 1_000, 0.1)])})
    assert out["flags"]["2891"]["exit_notes"] == {"00991A": "僅剩1張"}


def test_賣出後還剩不少_不註記():
    out = b.build_flags({"00991A": _snap(
        prev=[("2330", "台積電", 1000_000, 50.0), ("2891", "中信金", 50_000, 5.0)],
        today=[("2330", "台積電", 1000_000, 55.0), ("2891", "中信金", 30_000, 3.5)])})
    assert out["flags"]["2891"]["exit_notes"] == {}


def test_買超不會被誤標exit_notes():
    out = b.build_flags({"00981A": _snap(
        prev=[("2330", "台積電", 1000_000, 10.0)],
        today=[("2330", "台積電", 1200_000, 11.0)])})
    assert out["flags"]["2330"]["exit_notes"] == {}
