...
### ✅ D. ③ 產業逆風判定 —— 2026-09-08 完成
`screener/industry.py`：同產業近 6 月報酬中位數 < −10% **且**落後大盤 8pp（產業 ≥ 3 檔才判）
→ `industry_headwind` 旗標 + `industry_ret_6m`。**只顯示、不進 verdict**（比照循環高峰）。
價值+定存都掛，app 卡片臉上顯示「⚠ 產業逆風（近6月中位 -X%）」。實測 value 只中 3293。

**v1 收尾 ⑤/④/⑥/③ 全部完成。** 剩：實驗 B 報告重跑（腳本需 port）、`price_adjuster.py`/
本地進階模式即時補抓、個股頁三軌門檻檢視——都列 §E 日後拓展。

### E. 未來項（使用者提過、還沒定案）
- **`price_adjuster.py` 移植 / 本地進階模式即時補抓**：500 大以外個股的還原股價、
  FinMind 即時補抓。整塊 v1 沒做（雲端唯讀不需要）。PRD §M4 / §3.3。
- **AI 解說層**：設計備忘 `docs/AI_LAYER.md`（2026-09-08 起草，未實作）。三層引擎
  （本機 CLI headless 騎訂閱 / BYOK API key / 複製給 AI）+ 護欄（抄 `me/ai/analyzer.py` v4.0）
  + 盤前 briefing（美國收盤/法人流向）+ MCP server 方案。**AI 只解說、不選股**。
