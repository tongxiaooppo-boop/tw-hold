...
- [ ] **F1** 修 `fetch_fundamentals.py` 的 `capital_stock` / `cash` 補抓 → 重抓 balance（~500 req）
- [ ] **F2** FinMind `TaiwanStockInfo` 拿 `industry_category`（全市場 **1 req**）
- [ ] **F3** FinMind `TaiwanStockPrice` 抓未還原收盤，前 500 大 × 近 5 年（~500 req）
- [ ] 🔴 **除息跳空驗證**：`0056` / `2412` 的除息日，**未還原有跳空、還原沒有**。
      不對就停——填息率會恆等於 100% 且不報錯（守門員 G2）
- [ ] ⚠️ 避開 `fundamentals.yml` 排程（台灣週六 10:00），守 500/hr

### M-1.2 實驗 B · 組合層歷史回溯（2015 起，季換股）

- [ ] `research/backtest_rebalance.py`（**獨立腳本，不要用 tw-swing 的事件式引擎**）
- [ ] 價值線、定存線各跑；**N = 10 與 20 各一次**
- [ ] 四個數字：**含息年化 / 最大回撤 / 平均填息率 / 年換手率**
- [ ] 對照組：0050 含息、0056 含息、**「過硬門檻但不排序、等權全買」**（檢查排序有沒有貢獻）
- [ ] 🔴 公告日遞延要套（財報 +45 天、月營收 `available_date`）——**沒套 = 未來函數 = 作廢**
- [ ] 🔴 報告要寫明**生存者偏差**（宇宙是用今天的前 500 大抓的），結論當上界看
- [ ] 報告 → `docs/reports/rebalance_backtest_<日期>.md`

**M-1 驗收**：四個數字 × 兩條線 × 兩種 N × 對照組都在報告裡；
**回報使用者，由他決定 PRD v3.0 定不定案。**

> **三種結果三條路**：① 打得贏 0050 且回撤可接受 → 照 v3.0 動工；
> ② 只有定存線對標得上美元部位 → 只做定存，價值線砍掉；
> ③ 兩條都不如直接買 ETF → **誠實收掉清單功能，只留個股查詢頁**。
