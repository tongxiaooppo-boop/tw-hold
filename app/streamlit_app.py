...
SWING_DISCLAIMER = (
    "🟡 **候選池篩選規則本身已有回測驗證**（次日開盤買得到口徑、資金天花板約束，"
    "見下方「回測」區塊）——但**這裡顯示的停損位／可買上限是風控算術，不是被"
    "驗證過的買賣點**，只回答「這個進場點承擔多少風險」，**不回答「會不會賺」**。"
    "不給 verdict、不給總分排名。"
)
SHORT_DISCLAIMER = (
...
#: 但是**各自獨立顯示**，不是候選池那套「全過才算」的 AND 閘門，門檻數字也
#: 不完全相同（例如趨勢模板這裡最多 7/8，不含相對強弱 RS）。回測驗證的對象
#: 是候選池那套組合邏輯本身，不是這裡的逐項檢查，不能套用 SWING_DISCLAIMER
#: 那句「已有回測驗證」（2026-09-24 Opus 審核抓到：兩邊共用同一句文案會誤導）。
CHECKLIST_SWING_DISCLAIMER = (
    "🔴 **這裡逐項列出跟「長波段候選池」類似的檢查，但各自獨立顯示，不是候選池"
    "那套「全部條件都過才算」的組合邏輯**，門檻數字也不完全相同——回測驗證的"
    "對象是候選池那套組合本身，不是這裡的逐項檢查，只回答「這檔現在個別"
    "符不符合哪些條件」，不回答「符合幾項算不算數」。"
)
...
LABELS = {
    "ticker": "代號", "name": "名稱", "industry": "產業", "close": "現價", "market_cap": "市值",
    ...
    "first_seen": "首次進榜", "streak_days": "連續入選天數",
}
...
#: 排序選項——2026-09-24 使用者要求市值當預設排序依據（其他排序保留可選）。
#: dict 順序即顯示順序，`segmented_control` 的 `default=sort_opts[0]` 直接吃第一個 key。
SORT_KEYS = {
    "value": {"市值": "market_cap", "價值分數": "value_score", "空間%": "upside_pct",
              "現價": "close"},
...
BACKTEST_NOTES = {
    "value": {
        "logic": ("**這頁在看什麼**：台版 Magic Formula 式因子指數，品質 rank + 便宜 "
                 "rank，季換股（3/6/9/12 月，財報公告後），每檔給買入建議價或"
                 "「不推薦」＋原因。\n\n"
                 "**怎麼算的**：F-Score ≥ 6 + Magic Formula 精神（歸一化本益比／"
                 "盈餘殖利率排序），季換（3/31、5/15、8/14、11/14），前 15、單一"
                 "產業 ≤ 40%。verdict 只由便宜門檻驅動（PRD §6.3），不做總分排名式"
                 "的推薦。"),
        "glossary": (
        ...
        "backtest": (
            ...
            "| 年 | N=5 | N=10 | 0050 |\n"
            "| :-- | --: | --: | --: |\n"
            "| 2016 | +14.0%（228.1） | +19.3%（238.6） | +19.6%（228.6） |\n"
            "| 2017 | +7.9%（246.0） | +9.5%（261.3） | +18.1%（270.0） |\n"
            "| 2018 | −2.4%（240.1） | +5.5%（275.6） | −4.9%（256.7） |\n"
            "| 2019 | +2.5%（246.1） | +15.4%（318.1） | +33.5%（342.7） |\n"
            "| 2020 | +20.4%（296.3） | +20.0%（381.6） | +31.1%（449.5） |\n"
            "| 2021 | +32.7%（393.4） | +23.9%（472.7） | +21.9%（548.0） |\n"
            "| 2022 | −24.9%（295.5） | −21.2%（372.3） | −21.2%（431.7） |\n"
            "| 2023 | +17.0%（345.7） | +20.0%（446.8） | +27.5%（550.5） |\n"
            "| 2024 | +31.6%（454.8） | +22.0%（544.9） | +48.6%（818.3） |\n"
            "| 2025 | +30.4%（592.9） | +33.2%（725.6） | +36.9%（1,119.9） |\n"
            "| 2026 | +81.8%（1,078.1） | +84.5%（1,338.4） | +64.3%（1,839.5） |\n"
            "| **全期年化** | **+17.86%** | **+20.38%** | — |\n"
            "| **最大回撤** | −34.0% | −25.6% | — |\n"
            "| **MAR** | 0.52 | 0.80 | — |\n\n"
    ...
    "deposit": {
        "logic": ("**這頁在看什麼**：高股息低波動因子指數（想成 00713 式），殖利率＋"
                 "品質＋低波動＋填息率複合分。定位是接手美元收益部位，基準不是大盤"
                 "而是使用者自己賣掉的美元資產稅後殖利率。\n\n"
                 "**怎麼算的**：殖利率 ≥ 5%（目標 5.5%）+ 硬門檻（含填息率 ≥ 60%、"
                 "近 3 年含息報酬 ≥ 0），季換股，前 15、單一產業 ≤ 40%。買價 = 近 3 "
                 "年均現金股利 ÷ 殖利率門檻（PRD §7.3）。"),
        "backtest": (
    ...
    "swing": {
        "logic": ("**這頁在看什麼**：CANSLIM + Minervini 趨勢模板組成的「主動選股候選"
                 "池」，每日重算，狀態型條件全過才進池，不排名次不配權重。篩選規則"
                 "本身已有回測驗證，但風控推導的可買上限與失效條件不是。\n\n"
                 "**怎麼算的**：候選池六條件全過才進池：CANSLIM 兩條（季 EPS YoY>25%、"
                 "近3年TTM EPS成長）"
        ...
        "backtest": (
            ...
            "universe 609 檔，市況進場 × 移動停損組合矩陣，共 7 種口徑）**\n\n"
            ...
            "⚠️ **買得到口徑＝次日開盤，全 repo 唯一定義**（跟短線/價值分頁引用的"
            "「資金天花板」「每年歸零」那套 `backtest_scenario_b.py`／"
            "`backtest_top17_buyable.py` 一致）——tw-swing 之前實測過「限價三日」"
            "（訊號收盤掛限價、3日內沒成交放棄）當買得到候選，實測後打槍，確定"
            "買得到原則就是次日開盤。這支腳本早期另外把限價版本誤標成「買得到"
            "口徑」，2026-09-23 起這個限價變體已經整個移除，不再出現在任何回測"
            "矩陣或候選池邏輯裡。\n\n"
            "| 口徑 | 年化（2019起，正確） | 最大回撤 | 勝率 |\n"
            "| :-- | --: | --: | --: |\n"
            "| 次日開盤（＝買得到口徑，PRD字面規格：不限市況、固定停損） | +18.7% | −37.1% | 25% |\n"
            "| 次日開盤 + 移動 ATR 停損（不限市況） | +27.1% | −31.2% | 42% |\n"
            "| 只留多頭週（M gate）+ 固定停損 | +9.9% | −43.2% | 28% |\n"
            "| 只留多頭週（M gate）+ 移動 ATR 停損 | +10.0% | −31.6% | 41% |\n"
            "| 排除空頭週 + 固定停損 | +20.4% | −36.9% | 28% |\n"
            "| **✅ 排除空頭週 + 移動 ATR 停損（現行規格）** | **+31.0%** | **−29.4%** | 43% |\n"
            "| 排除空頭週 + 市況緊縮停損（空頭/震盪收緊到1.5×ATR） | +37.2% | −39.5% | 41% |\n"
            "| 0050 同期（含息還原，2019起） | +29.7% | −33.8% | — |\n\n"
            "**「排除空頭週 + 移動 ATR 停損」是七格裡的最佳解，已收進候選池的進出場邏輯"
            ...
            "| 年 | 策略 | 0050 |\n"
            "| :-- | --: | --: |\n"
            "| 2019 | −1.5%（196.9） | +33.5%（252.3） |\n"
            "| 2020 | +2.7%（202.2） | +31.1%（330.9） |\n"
            "| 2021 | +157.7%（521.1） | +21.9%（403.4） |\n"
            "| 2022 | −3.8%（501.4） | −21.2%（317.8） |\n"
            "| 2023 | +80.9%（907.1） | +27.5%（405.3） |\n"
            "| 2024 | +65.5%（1,501.3） | +48.6%（602.4） |\n"
            "| 2025 | +45.4%（2,183.2） | +36.9%（824.5） |\n"
            "| 2026 | +5.8%（2,309.8） | +64.3%（1,354.2） |\n"
            ...
            "`tw-hold/docs/reports/backtest_scenario_b_yearly_reset_2026-09-22.md`"
            "〈長波段候選池〉一節。\n\n"
            "⚠️ **差距很小、不要讀成穩贏**：2019 起窗口全期年化 +31.0% vs 0050 "
            "+29.7%，只差 1.25pp，而且 universe 是今天的 609 檔存活名單（生存者"
            "偏差），報告本身把這組數字當**上界**看待——比較精確的講法是「跟 "
            "0050 打平、上界小贏」，不是「穩定打贏大盤」。"),
    },
    "short": {
        ...
        # 跑的規則本身的邏輯，寫在這裡。
        "logic": (
            "**這頁在看什麼**：上面的候選清單是逐條攤開日線技術面條件、每日即時"
            "展示，本身**零回測零驗證**，不給訊號、不給買賣點——短線交易的守備"
            "範圍是姊妹專案 tw-swing，這裡只是資訊展示。下面則是不同的東西："
            "tw-swing 自己那套**有回測驗證**的規則（`capital_watch` 組合），純粹"
            "當背景資料參考，跟上面即時清單顯示的條件不是同一回事，不要混為一談。\n\n"
        ...
        "backtest": (
            ...
            "| :-- | --: | --: | --: | --: |\n"
            "| 2016 | −0.9%（99.1） | +92.8%（192.8） | +23.5%（123.5） | +19.6%（121.2） |\n"
            "| 2017 | −25.0%（74.3） | +24.3%（239.6） | +19.3%（147.4） | +18.1%（143.2） |\n"
            "| 2018 | −10.7%（66.3） | +79.9%（431.1） | −41.4%（86.3） | −4.9%（136.2） |\n"
            "| 2019 | +46.2%（96.9） | −19.9%（345.1） | −13.5%（74.7） | +33.5%（181.8） |\n"
            "| 2020 | +28.1%（124.2） | −15.0%（293.5） | −32.2%（50.6） | +31.1%（238.4） |\n"
            "| 2021 | +27.3%（158.1） | +39.4%（409.0） | +69.7%（85.9） | +21.9%（290.7） |\n"
            "| 2022 | +22.8%（194.2） | −20.3%（326.0） | −17.7%（70.7） | −21.2%（229.0） |\n"
            "| 2023 | +0.6%（195.3） | +9.5%（357.2） | +40.4%（99.2） | +27.5%（292.0） |\n"
            "| 2024 | −63.1%（72.1） | +25.2%（447.0） | +99.7%（198.2） | +48.6%（434.1） |\n"
            "| 2025 | +9.1%（78.7） | −45.6%（243.4） | +53.9%（305.0） | +36.9%（594.1） |\n"
            "| 2026 | +148.7%（195.7） | +48.1%（360.3） | −25.8%（226.4） | +64.3%（975.7） |\n\n"
...
def _swing_b_html(c: dict, flag: dict | None = None) -> str:
    ...
    streak = c.get("streak_days")
    streak_txt = ("🆕 今日新進榜" if streak == 1
                 else f"連續入選 {streak} 天（首次 {c.get('first_seen', '—')}）"
                 if streak else "")
    ctx = "　·　".join(x for x in [
        _esc(c.get("industry") or ""),
        f"市值 {_fmt('market_cap', c.get('market_cap'))}" if c.get("market_cap") is not None else "",
...
def _swing_page(payload: dict | None) -> None:
    ...
    sort_opts = list(SORT_KEYS["swing"])
    ...
    with st.expander("複製給 AI"):
        st.code(_copy_for_ai("長波段候選池", meta, rows),
                language="markdown")
    st.divider()
    _swing_exit_table()
    _swing_paper_section()
...
def _stock_page() -> None:
    ...
    st.caption("不打分，純數據 + plotly 圖表（K 線／營收等）。查詢範圍限 bundle 內"
              "的前 500 大約 1,000 檔，不即時抓單股資料（PRD 明定的紅線）。")
    _disclaimer()
    st.markdown("**股票代號**")
