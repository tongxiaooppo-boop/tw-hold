...
BACKTEST_NOTES = {
    "value": {
        ...
        "backtest": (
            ...
            "資金 100% 回籠，依市值由大到小只買前 N 檔。**N=5 年化 +17.86%（回撤 "
            "−34.0%、MAR 0.52，終值 1,078.1 萬），N=10 +20.38%（回撤 −25.6%、"
            "MAR 0.80，終值 1,338.4 萬）**，兩者都輸同期 0050（同組本金 200 萬"
            "對照組終值 1,839.5 萬）——N=5 輸得更多。\n\n"
    ...
    "swing": {
        ...
        "backtest": (
            ...
            "程式碼、不同模型）。**N=5 全期年化 +38.49%（回撤 −27.4%、MAR 1.41，終值"
            "2,309.8 萬），N=10 +38.80%（回撤 −27.4%、MAR 1.42，終值 2,348.6 萬）**——"
            "兩個模型方向一致（移動停損 + 市況篩選確實有效），資金天花板版數字更高是"
            "因為多頭年份（2021 +157.7%、2023 +80.9%）用複利部位重壓放大了效果，"
            "**兩份用不同方法論互相佐證同一個結論，不是互相矛盾**。完整數字："
    ...
    "short": {
        ...
        "backtest": (
            ...
            "**這是第三套模型**（風險%配置，不是資金天花板），三套(bench.html 逐"
            "規則篩選／tw-hold 資金天花板／tw-swing 風險%配置 tearsheet)各自回答"
            "不同問題，數字不能互相替代或加總。\n\n"
