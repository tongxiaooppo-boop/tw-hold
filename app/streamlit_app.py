...
BACKTEST_NOTES = {
    ...
    "swing": {
        "logic": ("候選池六條件全過才進池：CANSLIM 兩條（季 EPS YoY>25%、近3年TTM EPS成長）"
                 ...
                 "**不含大盤方向（CANSLIM 的「M」）**——2026-09-14 測過拿它當硬性 gate，"
                 "回測結果變差，決定不做，詳見下方名詞解釋與回測結果。"),
        "glossary": (
            ...
            "淨買超金額」當代理，不是「持股家數變化」 |\n"
            "| **M** 大盤方向 | 整體市場要處於確認的多頭 | ❌ **沒做，2026-09-14 測過"
            "當硬性 gate 後決定不做**——只在大盤多頭週才新進場，回測結果年化從 "
            "+13.2% 掉到 +7.0%、最大回撤從 −37.1% 惡化到 −43.2%（見下方回測結果），"
            "變差不是變好，所以連當 context flag 顯示在畫面上都沒有做，這裡純粹"
            "記錄「測過、沒用」 |\n\n"
        ...
        "backtest": (
            "**實驗 E（2026-09-14 重跑，事件驅動回測，2016-01→2026-09，550 週，"
            "universe 609 檔）**：\n\n"
            "| 口徑 | 年化 | 最大回撤 | 勝率 |\n"
            "| :-- | --: | --: | --: |\n"
            "| 次日開盤（PRD 字面規格：固定停損） | +13.2% | −37.1% | 25% |\n"
            "| 限價買得到（固定停損） | +11.7% | −37.1% | 23% |\n"
            "| 次日開盤 + 移動 ATR 停損（非本頁現行規格，對照組） | +19.3% | −31.2% | 42% |\n"
            "| 次日開盤 + CANSLIM「M」硬性 gate（只在大盤多頭週新進場） | +7.0% | "
            "**−43.2%** | 28% |\n"
            ...
            "| :-- | --: | --: | --: |\n"
            "| 次日開盤（現行規格） | +518.1% | +36.5% | −55.8% |\n"
            "| 限價買得到 | +455.8% | +29.1% | −54.5% |\n"
            "| 移動 ATR 停損（對照組） | +235.8% | **+210.6%** | −36.8% |\n"
            "| M 硬性 gate | +503.2% | **−24.2%** | −55.0% |\n\n"
            "獲利幾乎全部來自多頭（候選池本來就偏動能/趨勢股，正常）；空頭全部口徑都虧損，"
            "固定停損虧最深、移動停損虧最淺；**震盪期間才是四個口徑差異最大的地方**——"
            "移動停損版在震盪期間反而賺 +210.6%（部位放得比較久，吃到反彈），M gate 版"
            "在震盪期間直接虧 −24.2%（擋掉的正是進場較早、後續在震盪期撐住甚至獲利的"
            "那批交易，不是單純「少曝險=安全」這麼直覺）——**這是 M gate 讓整體表現"
            "變差的主要原因**，硬性擇時對這條策略是負貢獻，不是理論猜測，所以不只不"
            "gate，連 context flag 都沒有放在畫面上，只留這裡的文字記錄「測過、沒用」。\n\n"
            "完整結果：`docs/reports/backtest_longswing_20260912.md`"
            "（檔名沿用舊實驗，內容已更新到 2026-09-14）。\n\n"
